...
const register = async (req, res) => {
    ...
    const rc = await roleService.roleUser().then((result) => { return result } ) ;
    ...
    const user = new User({nom: nom, prenom: prenom, email: email, mdp: mdp, role: role});
...
async function logByMail(email) {
    return User.findOne({ email: email }).populate('role').then((result) => { 
        return result; 
    }); 
}
